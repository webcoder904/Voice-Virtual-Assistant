...
// User data
let userName = 'Sujal'; // Default user name
let tasks = [];
...
function wishMe() {
  ...
  speak(`${greeting} ${userName}. I'm your virtual assistant. How can I help you today?`);
}
...
async function takeCommand(message) {
  ...
  else if (message.includes('hey')) speak(`Hello ${userName}, How May I Help You?`);
  ...
  else if (message.includes('add task')) addTask(message);
  else if (message.includes('show tasks')) showTasks();
  else if (message.includes('fun fact')) tellFunFact();
...
async function takeCommand(message) {
  ...
  else if (message.includes('currency converter')) await convertCurrency(message);
  else if (message.includes('unit converter')) await convertUnit(message);
  else handleWebSearch(message);
}
...
async function convertCurrency(message) {
  const [amount, fromCurrency, toCurrency] = message.match(/(\d+)\s*(\w+)\s*to\s*(\w+)/).slice(1);
...
// Unit converter
async function convertUnit(message) {
  const [value, fromUnit, toUnit] = message.match(/(\d+)\s*(\w+)\s*to\s*(\w+)/).slice(1);
  ...
  if (fromUnit === 'meters' && toUnit === 'feet') {
    ...
    speak('Unit conversion not supported.');
    return;
  }
  speak(`${value} ${fromUnit} is equal to ${convertedValue} ${toUnit}.`);
}
